...
export class ImageService {
  ...
  images(username:string, currentUsername:string, paging:PagingParams):Observable<any>{
    return this.http.get<AppResponse>(
      MY_IMAGES,
      {
        params: new HttpParams().set('username', username).set('currentUsername', currentUsername).set('paging', JSON.stringify(paging))
      }
    ).map(
      data => {
        return data;
